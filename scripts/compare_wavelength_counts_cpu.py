...
def main():
    ...
    previous=json.loads((a.base/'search.json').read_text())
    ...
    old=dict(previous['baselines'][0])
    baseline8=dict(next(c for c in previous['selected'] if c['id']=='C2_worst_tail'))
    baseline8['id']='R8_C2'
    baselines=[old,baseline8]
    ...
    result=dict(kind='current_lut_wavelength_count_comparison_v1',dataset=str(a.dataset),model=meta['model'],
        ...
        counts=previous['counts'],region_validation_counts=previous['region_validation_counts'])
