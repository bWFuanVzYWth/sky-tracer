...
def main():
    ...
    if a.free_span:
        C = C[:3]
    old = tuple(int(np.flatnonzero(nm == F(v))[0]) for v in [400,440,480,520,560,600,640,680])
    baselines = []
    pool = []
    search_summary = []
    ...
    for oi, name in enumerate(['balanced', 'twilight', 'aureole', 'gradient', 'general']):
        ...
        if name == 'balanced':
            ...
            if old_refit is not None:
                baselines.append(dict(id='uniform_refit', indices=list(old), quadrature_weights_nm=(old_refit[1] * widths[list(old)]).tolist(),
    ...
    baselines.insert(0, dict(id='uniform_equal', indices=list(old), rgb_from_integrated=(W[list(old)] * (F(40) / widths[list(old)])[:,None]).tolist()))
