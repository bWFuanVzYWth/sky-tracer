...
use sky_pt::config::{OutputProjection, RenderConfig};
use sky_pt::integrator::render;
use sky_pt::physics::atmosphere::SceneData;
use sky_pt::physics::data::load_scene_data;
use sky_pt::physics::spectrum::{
    CIE_1931_2DEG_CMF_NAME, CIE_1931_2DEG_CMF_SOURCE, LINEAR_SRGB_D65_NAME, SpectralRgbConverter,
};

use crate::film_output::{read_film_from_band_exrs, write_outputs, write_rgb_outputs};
...
pub fn run(cli: Options) -> Result<(), Box<dyn Error>> {
    let total_start = Instant::now();
    let (width, height) = resolve_dimensions(cli.width, cli.height, cli.sky_view_lut);
    let out_dir = cli.out;
    let data_dir = cli
        .data_dir
        .unwrap_or_else(sky_pt::physics::data::default_data_dir);
...
fn write_asset_manifest(
    ...
    manifest.transport_version = Some(sky_assets::asset::LAYERED_TRANSPORT_VERSION.into());
