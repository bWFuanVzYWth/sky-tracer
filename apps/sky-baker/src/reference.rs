...
pub enum Command {
    ...
    /// CPU generation of total, single and boundary spectra for wavelength fitting.
    Spectra {
        source: PathBuf,
        #[arg(long)]
        queries: PathBuf,
    ...
    fn load(&self) -> Result<(Model, BakeConfig)> {
        let scene = sky_reference::physics::data::load_scene_data(
            &self
                .data_dir
                .clone()
                .unwrap_or_else(sky_reference::physics::data::default_data_dir),
            0.0,
            0.0,
        )
        ...
        let config = if let Some(path) = &self.config {
            ...
            match self.preset {
                Preset::Reference => BakeConfig::current_reference(),
...
pub fn run(command: Command) -> Result<()> {
    match command {
        Command::Spectra {
            ...
            steps,
            threads,
        } => sky_reference::spectral_dataset::export(&source, &queries, &out, steps, threads)?,
        Command::Plan {
            input,
            analyze_work,
        } => {
            ...
            let work = analyze_work.then(|| {
                let nodes = sky_reference::bake_schedule::angular_nodes(model.geometry, &config);
                sky_reference::bake_schedule::WorkEstimate::from_nodes(&config, &nodes)
            });
            println!(
                "{}",
        ...
        Command::Inspect { asset, verify } => {
            ...
            if verify {
                if m.rgb.is_some() {
                    sky_reference::rgb::verify(&m, &asset)?;
                }
        ...
        Command::ExportRgb { source, out } => {
            let m = sky_reference::rgb::export(&source, &out)?;
            sky_reference::rgb::verify(&m, &out)?;
        ...
        Command::CompressRgb {
            ...
            let m = sky_reference::packed::compress(&source, &out, block_texels)?;
            sky_reference::rgb::verify(&m, &out)?;
            println!(
        ...
        Command::Synthesize {
            ...
            let points: Vec<sky_reference::synthesis::SamplePoint> =
                serde_json::from_slice(&fs::read(queries)?)?;
            sky_reference::synthesis::export(&source, &points, &out)?;
        ...
        Command::Compare {
            ...
            compare(
                ...
                pt_max_orders,
                band_index,
                &data_dir.unwrap_or_else(sky_pt::physics::data::default_data_dir),
                width,
                height,
                spp,
                seed,
...
fn compare(
    ...
    let reference_data = sky_reference::physics::data::default_data_dir();
    for name in [
        "atmosphere_profile.csv",
        "aerosol_profile.csv",
        "aerosol_optics.csv",
        "bands.csv",
        "mie_phase.csv",
    ] {
        if fs::read(data.join(name))? != fs::read(reference_data.join(name))? {
            return Err(format!("PT/reference input tables differ: {name}; align independent input copies before comparison").into());
        }
    }
    let reference_scene = sky_reference::physics::data::load_scene_data(
        &sky_reference::physics::data::default_data_dir(),
        0.0,
        0.0,
    )
    .map_err(|e| e.to_string())?;
    let input_fingerprint = fingerprint(&Model::from_scene(&reference_scene)?)?;
    ...
    if (m.config.ground_albedo - sky_pt::physics::atmosphere::GROUND_ALBEDO).abs() > 1e-6 {
        return Err(
    ...
    let film = if let Some(band) = band_index {
        sky_pt::render_band(&scene, &render_config, band, pt_max_orders)?
    } else {
        match pt_max_orders {
            Some(n) => sky_pt::render_orders(&scene, &render_config, n)?,
    ...
    fs::write(
        ...
        "observer_altitude_km":altitude,"pt_seed":render_config.seed,"pt_transport_version":sky_assets::asset::LAYERED_TRANSPORT_VERSION,"pt_max_orders":pt_max_orders,"lut_pixel_quadrature":format!("{pixel_samples}x{pixel_samples}"),"excluded":"pixels intersecting solar disk bounding box",
