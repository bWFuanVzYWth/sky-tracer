...
use sky_assets::asset::{
    SPECTRAL_PANORAMA_KIND, SPECTRAL_SKY_VIEW_LUT_KIND, SpectralAssetManifest,
};
...
impl RealtimeAsset {
    /// UI defaults when no path-traced comparison asset was requested.
    pub fn demo_default() -> Self {
        Self {
            default_view: true,
            manifest_path: PathBuf::from("out/no-reference/asset.json"),
            root_dir: PathBuf::from("out/no-reference"),
            manifest: SpectralAssetManifest::spectral_panorama(
                [512, 256],
                0,
                0,
                20.0,
                ...
                0.2,
                Vec::new(),
                sky_assets::asset::SpectralAssetFiles {
                    rgb_exr: "missing.exr".into(),
                    rgb_png: "missing.png".into(),
                    band_exrs: Vec::new(),
                },
            ),
        }
    ...
    pub fn title(&self) -> String {
        if self.default_view {
            return "sky demo - no PT comparison loaded".into();
        }
        format!(
...
mod tests {
    use sky_assets::asset::{
        SPECTRAL_SKY_VIEW_LUT_KIND, SpectralAssetFiles, SpectralAssetManifest,
    };
