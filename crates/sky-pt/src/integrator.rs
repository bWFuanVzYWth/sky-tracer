...
use crate::physics::spectrum::BAND_COUNT;
...
mod tests {
    ...
    fn gpu_ground_bounce_matches_incident_hemisphere() {
        let scene =
            crate::physics::data::load_scene_data(&repository_data_dir(), -6.0, 0.0).unwrap();
...
"#);
    ...
    fn selected_band_matches_full_spectral_samples() {
        let scene =
            crate::physics::data::load_scene_data(&repository_data_dir(), 20.0, 0.0).unwrap();
        let config = RenderConfig {
            width: 8,
            height: 4,
            spp: 64,
...
"#,
        ...
        for y in 0..4 {
            ...
            let origin =
                crate::physics::math::Vec3::new(0.0, scene.planet.ground_radius_km + h, 0.0);
            ...
            for j in 0..16384 {
                tau += ds
                    * crate::physics::medium::coefficients_at(
                        &scene,
    ...
    fn packed_scene_matches_cpu_table_shapes() {
        ...
        let scene =
            crate::physics::data::load_scene_data(&repository_data_dir(), 0.0, 0.0).expect("scene");
