...
fn synthesis_rejects_invalid_query_coordinates() {
    let bad = sky_atmosphere_lut::synthesis::SamplePoint {
        altitude_km: 0.0,
...
fn cubic_view_stencil_never_reads_across_the_ground_sky_boundary() {
    let g = geometry();
    let c = BakeConfig {
        mapping: sky_atmosphere_lut::config::CoordinateMapping::RayAlignedReference,
...
fn explicit_scattering_heights_roundtrip_and_leave_tau_axis_independent() {
    ...
    for i in 0..c.scattering[0] {
        ...
        let value = sky_atmosphere_lut::ray_mapping::RayStencil::new_local_source(g, &c, s, None)
            .with_linear_radius()
