...
#[test]
fn gpu_horizon_reference_chart_preserves_thin_transport_and_vacuum()
-> sky_atmosphere_lut::Result<()> {
...
#[test]
fn gpu_ray_reference_direct_first_preserves_thin_transport() -> sky_atmosphere_lut::Result<()> {
    let gpu = GpuBaker::new()?;
    let model = homogeneous(1e-8);
    let c = BakeConfig {
        mapping: CoordinateMapping::RayAlignedReference,
        phase_interpolation: sky_atmosphere_lut::config::PhaseInterpolation::LogRadiance,
        view_interpolation: sky_atmosphere_lut::config::ViewInterpolation::MonotoneCubic,
        iteration_scheme: sky_atmosphere_lut::config::IterationScheme::FixedPoint,
        scattering: [5, 32, 33, 65],
        scattering_altitudes_km: vec![0.0, 1.0, 2.0, 30.0, 100.0],
        height_interpolation: sky_atmosphere_lut::config::HeightInterpolation::ReferenceCdf,
        source_mapping: sky_atmosphere_lut::config::SourceMapping::LocalLinearHeight,
        sun_mu: 3,
        sun_phi: 23,
        optical_depth: [32, 128],
        max_orders: 3,
...
#[test]
fn gpu_fixed_point_keeps_direct_surface_light_once() -> sky_atmosphere_lut::Result<()> {
    let gpu = GpuBaker::new()?;
...
#[test]
fn cpu_synthesis_streams_spectra_and_matches_band_queries() -> sky_atmosphere_lut::Result<()> {
    use sky_atmosphere_lut::{
        rgb::rec2020_weights,
    ...
    let rgb_manifest = sky_atmosphere_lut::rgb::export(&root, &rgb_root)?;
    let packed_manifest = sky_atmosphere_lut::packed::compress(&rgb_root, &packed_root, 16)?;
    let reopened = Manifest::open(&packed_root)?;
    assert_eq!(reopened.config, rgb_manifest.config);
    let packed = sky_atmosphere_lut::packed::PackedLut::read(&reopened, &packed_root)?;
    for channel in 0..3 {
        let (sun, radiance) =
            sky_atmosphere_lut::rgb::read_channel(&rgb_manifest, &rgb_root, channel)?;
        assert_eq!(sun, packed.sun[channel]);
        for (i, &v) in radiance.iter().enumerate() {
            assert_eq!(
                packed.fetch(i, channel).to_bits(),
                sky_atmosphere_lut::packed::quantize(v) << 12
            );
        }
    }
    assert!(sky_atmosphere_lut::packed::compress(&rgb_root, &packed_root, 16).is_err());
    assert!(sky_atmosphere_lut::packed::compress(&packed_root, &root.join("invalid"), 16).is_err());
    let map_path = packed_root.join("blocks.bin");
    let mut bytes = fs::read(&map_path)?;
    bytes[0] ^= 1;
    fs::write(&map_path, &bytes)?;
    assert!(sky_atmosphere_lut::packed::PackedLut::read(&packed_manifest, &packed_root).is_err());
...
fn mapped_solar_interpolation_preserves_positive_exponential_attenuation() {
    use sky_atmosphere_lut::config::SolarInterpolation;
    let dims = [2, 2, 9, 2];
...
fn actual_spectral_forward_peaks_survive_the_sun_aligned_grid() {
    let scene = sky_core::data::load_scene_data(
    ...
    for band_index in [6, 17, 30] {
        ...
        let phase = |nu: f32| {
            ...
            disk.iter()
                .map(|&(v, w)| {
                    w * sky_atmosphere_lut::model::phase_weight(
                        coeff,
...
-> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let rgb_root = root.join("rgb");
    let rgb_manifest = sky_atmosphere_lut::rgb::export(&root, &rgb_root)?;
    sky_atmosphere_lut::rgb::verify(&rgb_manifest, &rgb_root)?;
    assert!(rgb_manifest.read_band(&rgb_root, 0).is_err());
    assert!(sky_atmosphere_lut::rgb::export(&rgb_root, &root.join("reexport")).is_err());
    let weights = sky_atmosphere_lut::rgb::rec2020_weights(&manifest);
    for channel in 0..3 {
        let (solar, values) =
            sky_atmosphere_lut::rgb::read_channel(&rgb_manifest, &rgb_root, channel)?;
    ...
    bytes[10] ^= 1;
    fs::write(&path, bytes)?;
    assert!(sky_atmosphere_lut::rgb::verify(&rgb_manifest, &rgb_root).is_err());
    for file in [
        "channel_0.bin",
        "channel_1.bin",
        "channel_2.bin",
