...
const TRANSMITTANCE_STEPS: u32 = 32u;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    let ray_origin = vec3<f32>(0.0, 0.0, radius_km);
    let sun_dir_local = vec3<f32>(-sin_theta, 0.0, mu);
    let t_max = ray_sphere_intersection(ray_origin, sun_dir_local, top);
    let dt = t_max / f32(TRANSMITTANCE_STEPS);

    var optical_depth = vec4<f32>(0.0);
    for (var i: u32 = 0u; i < TRANSMITTANCE_STEPS; i = i + 1u) {
        let t = (f32(i) + 0.5) * dt;
        let pos = ray_origin + sun_dir_local * t;
