...
fn linear_rec2020_from_spectral(l: vec4<f32>) -> vec3<f32> {
    // 410/440/480/520 nm low group -> scene-linear Rec.2020. The columns are direct CIE 1931
    // 2 degree CMF samples transformed to Rec.2020 and multiplied by the
    // cmf-moments fixed-sum quadrature weights:
    // 92.8257/20.8733/60.1133/20.884 nm.
    let m = mat4x3<f32>(
        vec3<f32>(2.015521915, -2.20749785, 18.203864142),
        vec3<f32>(3.069387571, -3.495544704, 34.463233423),
        vec3<f32>(-5.484677337, 10.446526205, 45.783739329),
        vec3<f32>(-3.419542345, 23.113484232, 0.928681094),
    );
    return m * l;
}

fn linear_rec2020_from_spectral8(lo: vec4<f32>, hi: vec4<f32>) -> vec3<f32> {
    // 410/440/480/520 + 540/580/610/670 nm searched 8-wave quadrature.
    let m_lo = mat4x3<f32>(
        vec3<f32>(2.015521915, -2.20749785, 18.203864142),
        vec3<f32>(3.069387571, -3.495544704, 34.463233423),
        vec3<f32>(-5.484677337, 10.446526205, 45.783739329),
    ...
    let m_hi = mat4x3<f32>(
        vec3<f32>(4.243623958, 37.153565117, -0.456028883),
        vec3<f32>(55.000002375, 34.63775886, -0.84876689),
        vec3<f32>(47.02149692, 4.411417716, -0.106950912),
        vec3<f32>(15.768316051, -0.743849551, 0.019681577),
    );
    return m_lo * lo + m_hi * hi;
}
...
fn white_balance_rec2020(rgb: vec3<f32>) -> vec3<f32> {
    ...
    // the sparse 8-wave solar samples.
    let m = mat3x3<f32>(
...
fn white_balanced_linear_rec2020_from_spectral8(lo: vec4<f32>, hi: vec4<f32>) -> vec3<f32> {
