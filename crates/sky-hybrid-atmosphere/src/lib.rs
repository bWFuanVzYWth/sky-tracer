...
pub use sky_atmosphere_lut::{Result, renderer::View};
use sky_atmosphere_lut::{mapping::unit, model::Model, quadrature, reference_mapping};
...
impl Medium {
    ...
    fn new(model: &Model, w: &Wavelengths, c: &Config, albedo: f32) -> Result<Self> {
        ...
        *heights.last_mut().unwrap() = g.top_height();
        ...
        let phase_mass = w
            .indices
            .map(|i| sky_atmosphere_lut::anisotropic::phase_moments(&model.bands[i], 0));
        for species in 1..5 {
        ...
        let key = sky_atmosphere_lut::four_wave::checksum(&key_bytes);
