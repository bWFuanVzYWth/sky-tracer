...
use clap::Parser;
use glam::Vec4;
use sky_atmosphere_lut::{mapping::State, model::Model, solver::GpuBaker};
use sky_hybrid_atmosphere::{Config, Renderer, Result, View, Wavelengths, read_texture};
use std::path::{Path, PathBuf};
#[derive(Parser)]
struct Args {
    #[arg(long, default_value = "out/hybrid_v1/invariants.json")]
    out: PathBuf,
}
...
fn main() -> Result<()> {
    ...
    let changed = Model::from_scene(&changed)?;
