...
use clap::Parser;
use sky_hybrid_atmosphere::{Config, Renderer, Result, View, Wavelengths, read_texture};
...
struct Args {
    ...
    #[arg(long, default_value = "out/wavelength_counts_v1/N4_best.json")]
    wavelengths: PathBuf,
...
async fn run() -> Result<()> {
    ...
    let w: Wavelengths = serde_json::from_slice(&fs::read(&a.wavelengths)?)?;
    let mut scene = sky_core::data::load_scene_data(std::path::Path::new("data"), 0.0, 0.0)
        .map_err(|e| e.to_string())?;
    if !a.aerosol_scale.is_finite() || !(0.0..=16.0).contains(&a.aerosol_scale) {
        return Err("invalid aerosol stress-test multiplier".into());
    }
    for point in &mut scene.aerosol_profile {
        for mass in &mut point.mass_g_m3 {
            *mass *= a.aerosol_scale;
        }
    }
    ...
    fs::write(
        ...
        serde_json::to_vec_pretty(
            &serde_json::json!({"config":c,"aerosol_scale":a.aerosol_scale,"wavelengths":w,"adapter":adapter.get_info().name,"pipeline_create_ms":compile_ms,"solve":solve,
                "mapping_calibration":serde_json::from_str::<serde_json::Value>(include_str!("../configs/mapping_fit.json"))?,
                "shader_checksum":sky_atmosphere_lut::four_wave::checksum(sky_hybrid_atmosphere::shader_source().as_bytes())}),
        )?,
    )?;
    if a.export_source {
