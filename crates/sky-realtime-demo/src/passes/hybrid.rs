...
use sky_hybrid_atmosphere::{Config, Renderer, View, Wavelengths};
...
pub struct HybridExperiment {
    ...
    scene: sky_core::atmosphere::SceneData,
...
impl HybridExperiment {
    pub fn new(context: ExperimentInit<'_>) -> Result<Self, String> {
        ...
        let model =
            sky_atmosphere_lut::model::Model::from_scene(&scene).map_err(|e| e.to_string())?;
...
impl RealtimeExperiment for HybridExperiment {
    ...
    fn render(&mut self, context: FrameContext<'_>) {
        ...
        if requested != self.applied_medium {
            let mut changed = self.scene.clone();
            for point in &mut changed.aerosol_profile {
                for mass in &mut point.mass_g_m3 {
                    *mass *= requested[0];
                }
            }
            let result = sky_atmosphere_lut::model::Model::from_scene(&changed).and_then(|model| {
