...
use sky_bruneton_atmosphere_8wave::{
    AerosolPreset, BrunetonAtmosphereContext, BrunetonFrameParams, Gpu, HillaireAtmosphere,
    HillairePhaseMode, HillaireSettings, NonZeroRenderSize, RenderTargets,
    SUN_IRRADIANCE_REC2020_W_PER_M2, Sun, ViewFrame,
};
...
pub struct BrunetonAtmosphere8WaveExperiment {
    atmosphere: BrunetonAtmosphereContext,
...
impl BrunetonAtmosphere8WaveExperiment {
    pub fn new(context: ExperimentInit<'_>) -> Result<Self, String> {
        ...
        let atmosphere = BrunetonAtmosphereContext::new(&gpu).map_err(|error| error.to_string())?;
...
impl RealtimeExperiment for BrunetonAtmosphere8WaveExperiment {
    fn name(&self) -> &'static str {
        "bruneton-8wave-sky-atmosphere"
    }
    ...
    fn render(&mut self, context: FrameContext<'_>) {
        ...
        let frame_params = BrunetonFrameParams {
