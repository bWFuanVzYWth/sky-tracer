...
struct PresentParams {
    ...
    tonemap_hdr_reinhard_white: vec4<f32>,
    hdr_peak_reserved: vec4<f32>,
};
...
fn display_linear_srgb_from_scene_rec2020(scene_rec2020: vec3<f32>) -> vec3<f32> {
    ...
    let linear_srgb = max(rec2020_to_srgb(scene), vec3<f32>(0.0));
    let tone_mapping = present.tonemap_hdr_reinhard_white.x > 0.5;
    let hdr = present.tonemap_hdr_reinhard_white.y > 0.5;
    let overexposure = present.tonemap_hdr_reinhard_white.z;
    let paper_white_scale = present.tonemap_hdr_reinhard_white.w;
    let peak_scale = present.hdr_peak_reserved.x;
    var mapped = linear_srgb;
    if (tone_mapping) {
        mapped = reinhard_gamut_map(linear_srgb, overexposure);
    }
    if (hdr) {
        return clamp(mapped * paper_white_scale, vec3<f32>(0.0), vec3<f32>(peak_scale));
    }
